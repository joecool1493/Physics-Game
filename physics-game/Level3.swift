...
class Level3: SKScene, SKPhysicsContactDelegate {
    ...
    let triangleBlock = SKSpriteNode(imageNamed: "triangleBlock")
    let swipeBlock = SKSpriteNode(imageNamed: "swipeBlock")
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let touch = touches.first{
            ...
            if touchedNode.name == "ball" {
                ...
                horizontalBlock.physicsBody?.affectedByGravity = true
    ...
    func setupNodes() {
        ...
        bowlingBall.position = CGPoint(x: bowlingBall.size.width, y: self.size.height * 0.85)
        ...
        triangleBlock.position = CGPoint(x: bowlingBall.position.x, y: self.size.height * 0.35)
        triangleBlock.anchorPoint = CGPoint(x: 0.5, y: 0.5)
        let offsetX = triangleBlock.size.width * triangleBlock.anchorPoint.x
        let offsetY = triangleBlock.size.height * triangleBlock.anchorPoint.y
        let path = CGMutablePath()
        path.move(to: CGPoint(x: 168 - offsetX, y: 0 - offsetY))
        path.addLine(to: CGPoint(x: 0 - offsetX, y: 168 - offsetY))
        path.addLine(to: CGPoint(x: 0 - offsetX, y: 0 - offsetY))
        path.closeSubpath()
        triangleBlock.physicsBody = SKPhysicsBody(polygonFrom: path)
        triangleBlock.physicsBody?.affectedByGravity = false
        triangleBlock.physicsBody?.isDynamic = false
        addChild(triangleBlock)
        
        horizontalBlock.position = CGPoint(x: bowlingBall.position.x - horizontalBlock.size.width * 0.4, y: triangleBlock.position.y * 1.6)
        ...
        horizontalBlock.physicsBody?.affectedByGravity = false
        addChild(horizontalBlock)
        
        swipeBlock.position = CGPoint(x: self.size.width * 0.8, y: self.size.height * 0.75)
        swipeBlock.anchorPoint = CGPoint(x: 0.5, y: 0.5)
        swipeBlock.name = "swipeBlock"
        swipeBlock.physicsBody = SKPhysicsBody(rectangleOf: swipeBlock.frame.size)
        swipeBlock.physicsBody?.affectedByGravity = false
        swipeBlock.physicsBody?.isDynamic = false
        addChild(swipeBlock)
        
        redButton.position = CGPoint(x: self.size.width * 0.75, y: 20)
    ...
    func setupGestureRecognizer() {
        ...
        let swipeDown = UISwipeGestureRecognizer(target: self, action: #selector(Level3.swiped(_:)))
        swipeDown.direction = .down
        self.view?.addGestureRecognizer(swipeDown)
        
        let swipeUp = UISwipeGestureRecognizer(target: self, action: #selector(Level3.swiped(_:)))
        swipeUp.direction = .up
        self.view?.addGestureRecognizer(swipeUp)
        
        let swipeLeft = UISwipeGestureRecognizer(target: self, action: #selector(Level3.swiped(_:)))
        swipeLeft.direction = .left
        self.view?.addGestureRecognizer(swipeLeft)
        
        let swipeRight = UISwipeGestureRecognizer(target: self, action: #selector(Level3.swiped(_:)))
    ...
    @objc func swiped(_ gesture: UIGestureRecognizer) {
        ...
        let moveAmount : CGFloat = 100.0
        let duration = 0.25
        
        if let swipeGesture = gesture as? UISwipeGestureRecognizer {
            switch swipeGesture.direction {
            case UISwipeGestureRecognizerDirection.right:
                run(SKAction.playSoundFileNamed("whoosh.mp3", waitForCompletion: false))
                swipeBlock.run(SKAction.moveBy(x: moveAmount, y: 0, duration: duration))
            ...
            case UISwipeGestureRecognizerDirection.up:
                run(SKAction.playSoundFileNamed("whoosh.mp3", waitForCompletion: false))
                swipeBlock.run(SKAction.moveBy(x: 0, y: moveAmount, duration: duration))
